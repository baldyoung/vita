...
var BillFilterModule = {
	diningRoomBuffer : [],
	init : function() {
		var data = BillFilterModule.requestDiningRoomData();
		BillFilterModule.diningRoomBuffer = data;
		$('#filterDiningRoom').html('<option value="all">全部</option> ');
		for (var i=0; i<data.length; i++) {
			var cell = data[i];
			var temp = '<option value="'+cell+'">'+cell+'</option>';
			$('#filterDiningRoom').append(temp);
		}
	},
	reset : function() {
		$('#filterDiningRoom').val('all');
	...
	packageData : function() {
		var data = {
			noZeroBill : $('#filterNoZeroBill').hasClass("checked") ? 1 : 0,
			zeroBill : $('#filterZeroBill').hasClass("checked") ? 1 : 0,
			unReceiveBill : $('#filterUnReceiveBill').hasClass("checked") ? 1 : 0
		}
		if ($('#filterDiningRoom').val() != 'all') {
			data.diningRoom = $('#filterDiningRoom').val();
		}
		return data;
	},
	click : function(tId) {
	...
	requestDiningRoomData : function() {
		var targetData = [];
		// ajax 请求
		return targetData;
	}
