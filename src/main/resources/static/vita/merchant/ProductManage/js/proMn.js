...
var ProductEditModule = {
    ...
    packageData: function () { // 获取编辑区的商品数据
        var temp = {
            ...
            productStock: 0, // $('#newProductStock').val(),
            productInfo: $('#newProductRemark').val(),
            // productIsShow: 0,
            productStockFlag: 0, // $('#needStock').is(':checked') ? 0 : 1,
        };
        ...
        dataTable.DataTable({
            ...
            "columnDefs": [{
                "render": function (data, type, row) {
                    var a = "";
                    a += "<button class=\"btn btn-primary \" style='margin-top:3px;' type=\"button\"  onclick=\"OptionModule.deleteProduct('" + row.productId + "', '" + row.productName + "')\"><i class=\"fa fa-trash\"></i> 删除"
                    a += "</button>"
                ...
                "render": function (data, type, row) {
                    return '<span style="font-weight:bold; color:black;">' + row.productName + "</span>";
                },
                "targets": 2
            }, {
                "render": function (data, type, row) {
                    if (row.productIsShowStatus == '上架') {
                        return '<span style="color:green;">' + row.productIsShowStatus + "</span>";
                    }
                    return '<span style="color:red;">' + row.productIsShowStatus + "</span>";
                },
                "targets": 6
            }]
        });
